...
(function (global) {
    ...
    /*
     * A timetable is not an ordinary settings blob: a mistaken cloud copy can
     * make another madrasa's teachers/classes appear in this one.  It is read
     * only through Attendance's ownership-checked reader in attendance.js.
     * Writes remain enabled through the durable outbox / writeModuleKey path.
     */
    ...
    function queueUpsert(key, value) {
        if (typeof global.emsOfflineEnqueueSyncModule === 'function') {
            return global.emsOfflineEnqueueSyncModule(key, value, {
                tenantId: state.uid,
    ...
    function writeToFirestore(uid, key, value) {
        ...
        return ref.set({
            ...
            emitSyncEvent('sync_ok', { key: key, path: (typeof global.emsGetTenantRootCollection === 'function' ? global.emsGetTenantRootCollection() : 'All_Madrasas') + '/' + uid + '/ModuleData/' + (SYNC_REGISTRY[key] || 'General') + '__' + key });
            return true;
        });
    }
    ...
    function updateSyncBanner() {
        if (typeof global.emsSyncFailureRefreshUi === 'function') {
            global.emsSyncFailureRefreshUi({
                ...
                deadLetter: state.deadLetterCount || 0,
    ...
    function refreshPendingCount() {
        if (typeof global.emsOfflineGetSyncFailureState === 'function') {
            return global.emsOfflineGetSyncFailureState().then(function (st) {
                state.pendingCount = st.pending || 0;
                state.failedCount = st.failed || 0;
                updateSyncBanner();
    ...
    function flushQueue() {
        if (typeof global.emsOfflineFlushAll === 'function') {
            ...
            return global.emsOfflineFlushAll({ manual: true }).then(function (res) {
                ...
                state.pendingCount = res.pending != null ? res.pending : 0;
                state.failedCount = res.queueFailed != null ? res.queueFailed : (res.failed || 0);
                updateSyncBanner();
                return { synced: res.flushed || 0, pending: state.pendingCount, failed: state.failedCount };
            }).catch(function (err) {
                state.flushing = false;
    ...
    function pullModuleKey(uid, key, options) {
        ...
        if (typeof global.emsMayPullFromCloud === 'function' && !global.emsMayPullFromCloud(options)) {
            return Promise.resolve({ changed: false, skipped: true, reason: 'pull_blocked' });
        }
        var db = getDb();
        if (!db || !isActiveSyncKey(key)) {
            return Promise.resolve({ changed: false, skipped: true, reason: 'inactive_key' });
        }
        return firestoreModuleRef(uid, key).get({ source: 'server' }).then(function (doc) {
            if (!doc.exists) {
                if (options.deltaOnly && global.EmsCachePolicy && typeof global.EmsCachePolicy.setPullCursor === 'function') {
                    global.EmsCachePolicy.setPullCursor(key, 0, { force: true });
                }
                return { changed: false, skipped: true, reason: 'no_doc' };
            }
            var remote = doc.data();
            if (!remote || remote.data == null) {
                return { changed: false, skipped: true, reason: 'empty_remote' };
            }
            var remoteStr = typeof remote.data === 'string' ? remote.data : JSON.stringify(remote.data);
            var local = null;
            if (typeof global.emsCacheGetRaw === 'function') {
                local = global.emsCacheGetRaw(key);
            } else if (typeof global.emsDurableReadRaw === 'function' && typeof global.emsIsLargeBlobKey === 'function' && global.emsIsLargeBlobKey(key)) {
                local = global.emsDurableReadRaw(key);
            } else {
            ...
            if (options.deltaOnly && global.EmsCachePolicy && typeof global.EmsCachePolicy.getPullCursor === 'function') {
                var cursor = global.EmsCachePolicy.getPullCursor(key) || 0;
                if (cursor && remoteAt <= cursor && local === remoteStr) {
                    return { changed: false, skipped: true, reason: 'delta_unchanged', key: key, remoteAt: remoteAt };
                }
            }
            ...
            if (decision.markSync && global.EmsCachePolicy) {
                ...
                return { changed: false, skipped: true, reason: 'identical', key: key };
            }
            if (!decision.apply) {
                return { changed: false, skipped: true, reason: decision.reason || 'local_pending', key: key };
            }
            ...
            if (global.EmsCachePolicy) {
                global.EmsCachePolicy.markSynced(key, remoteAt);
                if (options.deltaOnly && typeof global.EmsCachePolicy.setPullCursor === 'function') {
                    global.EmsCachePolicy.setPullCursor(key, remoteAt);
                }
            }
            return { changed: true, key: key, remoteAt: remoteAt };
        }).catch(function (err) {
            console.warn('[EMS] pullModuleKey failed', key, err && err.message ? err.message : err);
            return { changed: false, skipped: true, reason: 'error', key: key };
        });
    }
    ...
    function applyLocalFromRemote(key, remoteStr, tenantId) {
        ...
        if (!physical) {
            ...
            console.warn('[EMS] pull apply blocked — no tenant partition for', key);
            return;
        }
        if (typeof global.emsOfflineWriteLocalSync === 'function') {
            ...
            var wrote = global.emsOfflineWriteLocalSync(key, payload, { tenantId: tenantId });
            global._emsSuppressSync = false;
            if (!wrote) {
                console.warn('[EMS] pull apply blocked — tenant bound write failed', key);
                return;
            }
        } else if (typeof global.emsIsLargeBlobKey === 'function' && global.emsIsLargeBlobKey(key)
            && typeof global.emsDurableWriteRaw === 'function') {
            global.emsDurableWriteRaw(physical, remoteStr);
            global._emsSuppressSync = false;
        ...
        if (typeof global.emsCacheInvalidate === 'function') {
            global.emsCacheInvalidate(physical);
            global.emsCacheInvalidate(key);
        }
        if (global.EmsCachePolicy) global.EmsCachePolicy.touchKey(key);
        emitSyncEvent('pull_ok', { key: key, tenantId: tenantId, physicalKey: physical });
    }
    ...
    function pullAllModules(uid, options) {
        options = options || {};
        if (typeof global.emsMayPullFromCloud === 'function' && !global.emsMayPullFromCloud(options)) {
            return Promise.resolve({ pulled: 0, skipped: 0, source: 'pull_blocked_offline_first' });
        }
        var db = getDb();
        if (!db || !uid) return Promise.resolve({ pulled: 0, skipped: 0 });
        ...
        keys.forEach(function (key) {
            chain = chain.then(function () {
                return pullModuleKey(uid, key, options).then(function (res) {
                    if (res && res.changed) pulled++;
                    else skipped++;
                });
            });
        });
    ...
    function migrateLegacyBackup(uid) {
        ...
        return tenantDocRef(db, uid).collection('Backup').limit(100000).get()
            .then(function (snap) {
    ...
    global.EmsSyncEngine = {
        ...
        onLocalWrite: function (key, value) {
            ...
            if (global.EmsCachePolicy) global.EmsCachePolicy.touchKey(key);
            return persistKey(key, value, {});
        },

        persistAndSync: persistKey,
        ...
        getFirestorePath: function (key) {
            ...
            return (typeof global.emsGetTenantRootCollection === 'function' ? global.emsGetTenantRootCollection() : 'All_Madrasas') + '/' + state.uid + '/ModuleData/' + SYNC_REGISTRY[key] + '__' + key;
        },
        ...
        writeModuleKey: function (uid, key, value) {
            return writeToFirestore(uid || state.uid, key, value);
        }
    };
